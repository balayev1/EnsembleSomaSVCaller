...
process HETPILEUPS {
    ...
    val(filter)                     // If TRUE, keep only candidate hets in the output: Default is TRUE
    val(max_depth)                  // Maximum read depth for each SNP: Default is 500
    path(hapmap_sites)              // Path to .vcf file containing SNPs to probe
