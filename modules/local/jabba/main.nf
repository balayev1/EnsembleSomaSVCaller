process JABBA {
    ...
    container "oras://quay.io/balay011/jabba-r45-gurobi1300:0.2.0"


    input:
    tuple val(meta), path(junction), path(cov_rds), val(j_supp), path(het_pileups_wgs), val(purity), val(ploidy), path(cbs_seg_rds), path(cbs_nseg_rds)
    ...
    "${task.process}":
        JaBbA: \$(Rscript -e "library(JaBbA); cat(as.character(packageVersion('JaBbA')))")
    END_VERSIONS
    """
    ...
    """
    touch ${prefix}.jabba.simple.rds
    touch ${prefix}.jabba.simple.gg.rds
    touch ${prefix}.jabba.simple.vcf
    touch ${prefix}.jabba.raw.rds
    touch ${prefix}.opt.report.rds
    touch ${prefix}.jabba.seg
    touch ${prefix}.karyograph.rds
...
process RETIER_WHITELIST_JUNCTIONS {
    ...
    conda (params.enable_conda ? "bioconda::r-base=4.1.0" : null)
    container "oras://quay.io/balay011/jabba-r45-gurobi1300:0.2.0"
