...
workflow BAM_DELLY {
    ...
    delly_samples_input = delly_call_bcf.join(delly_call_bcf_csi)
    CREATE_DELLY_SAMPLES_TSV(delly_samples_input)
    delly_samples_tsv = CREATE_DELLY_SAMPLES_TSV.out.tsv

    filter_input = delly_samples_input.join(delly_samples_tsv)
    DELLY_FILTER(
        filter_input,
        mode,
        min_qual,
